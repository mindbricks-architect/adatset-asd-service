...
console.log("Starting adatset-asd-service service...");
...
const start = async () => {
  process.title = "node-adatset-asd-service";
  ...
  try {
    ...
    await setRedisData("adatset-asd-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key adatset-asd-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "adatset-asd-service is listening HTTP/REST port " + servicePort.toString(),
  );
  console.log(
...
/* starting the server adatset-asd-service .... */
start();
